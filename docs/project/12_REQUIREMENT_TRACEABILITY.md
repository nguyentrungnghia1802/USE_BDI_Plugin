...
| FR-DIA-006 | `BdiDiagramPanel`, `BdiDiagramCanvas`, deterministic Swing layout, and BDI-focused view modes | `BdiDiagramPanelTest`, `BdiExplorerViewTest`, and `DiagramModeProjectorTest` cover empty/populated/large/selection/zoom/state cases, mode switching without reparsing, selection retention, plan-step order, confirmed mappings, and explicit gaps; highlighting remains open | ADR-0038/0039, architecture, technical design |
