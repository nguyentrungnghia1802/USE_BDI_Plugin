...
| FR-DIA-008 | four canonical demo bundles and Diagram walkthroughs are implemented; export, mutant, screenshot, and performance evidence remain open | `CanonicalDemoDiagramTest` builds Family Person, Smart Queue, Smart Home, and Auction from user-facing files; `DocumentationContractTest` enforces complete bundles and walkthrough controls | architecture, technical design, product requirements, checklist |
