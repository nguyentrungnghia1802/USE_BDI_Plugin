...
| FR-DIA-006 | `BdiDiagramPanel`, `BdiDiagramCanvas`, deterministic Swing layout, BDI-focused view modes, and issue/mapping highlighting | `BdiDiagramPanelTest`, `BdiExplorerViewTest`, `DiagramModeProjectorTest`, `DiagramVisualStateTest`, `DiagramHighlightPathTest`, and `BdiProblemPanelTest` cover empty/populated/large/selection/zoom/state cases, mode switching without reparsing, selection retention, plan-step order, confirmed mappings, explicit gaps, issue evidence paths, Problems-to-Diagram selection, and UNKNOWN/STALE/MISSING states; broader navigation remains open | ADR-0038/0039/0040, architecture, technical design |
