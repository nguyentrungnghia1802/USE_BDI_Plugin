...
| FR-DIA-008 | four canonical demo bundles and reviewed Auction `MAP-003`/`SIG-001`/`REF-001`/`OCL-001` visual paths are implemented; export, screenshot, and performance evidence remain open | `CanonicalDemoDiagramTest`; `AuctionMutantDiagramTest` runs the real headless snapshots from the reviewed manifest and rejects sibling issue branches; `DocumentationContractTest` enforces complete bundles and walkthrough controls | architecture, technical design, product requirements, checklist |
